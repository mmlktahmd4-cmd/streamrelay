...
const sourceUrlField = z.preprocess(
  (v) => (typeof v === 'string' ? v.trim() : v),
  z.string().min(8, 'Source URL too short').max(2048)
).superRefine((url, ctx) => {
  if (url.startsWith('udp://')) return;
  try {
    const u = new URL(url);
    if (!['http:', 'https:', 'rtmp:', 'rtmps:'].includes(u.protocol)) {
      ctx.addIssue({ code: 'custom', message: 'Unsupported source URL scheme' });
    }
  } catch {
    ctx.addIssue({ code: 'custom', message: 'Invalid source URL format' });
  }
});

const backupSourceUrlField = z.preprocess(
  (v) => (typeof v === 'string' ? v.trim() : v),
  ...
  try {
    const u = new URL(url);
    if (!['http:', 'https:', 'rtmp:', 'rtmps:'].includes(u.protocol)) {
      ctx.addIssue({ code: 'custom', message: 'Unsupported backup URL scheme' });
    }
  } catch {
    ctx.addIssue({ code: 'custom', message: 'Invalid backup URL format' });
  }
});
...
export const importM3UUrlPreviewSchema = z.object({
  url: z.string().min(10).max(2048),
});

export const importM3USelectedSchema = z.object({
  channels: z.array(z.object({
    name: z.string().min(1).max(255),
    url: z.string().min(8).max(2048),
    logo_url: z.string().max(512).optional().nullable(),
    group: z.string().max(128).optional().nullable(),
    epg_id: z.string().max(128).optional().nullable(),
  })).min(1).max(2000),
  category_id: z.string().uuid().optional(),
  is_public: z.boolean().default(true),
  on_demand: z.boolean().default(false),
...
export const bulkDeleteChannelsSchema = z.object({
  ids: z.array(z.string().uuid()).optional(),
  all: z.boolean().optional(),
}).refine((data) => data.all || (data.ids && data.ids.length > 0), {
  message: 'Provide ids or set all=true',
...
export const bulkChannelIdsSchema = z.object({
  ids: z.array(z.string().uuid()).optional(),
  all: z.boolean().optional(),
