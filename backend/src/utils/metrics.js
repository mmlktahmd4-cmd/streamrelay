...
export async function probeHlsManifest(channelId, maxAgeMs = 45000) {
  const dir = path.join(config.streaming.hlsDir, channelId);
  const filePath = path.join(dir, 'index.m3u8');

  try {
    const stat = await fs.stat(filePath);
    let ageMs = Date.now() - stat.mtimeMs;
    if (ageMs <= maxAgeMs) {
      return { alive: true, age_ms: ageMs, path: filePath };
    }
    ...
    try {
      ...
      return { alive: false, age_ms: ageMs, path: filePath };
    }

    const segments = files.filter((f) => f.endsWith('.ts') || f.endsWith('.m4s'));
    if (segments.length > 0) {
      let newestMtime = 0;
      for (const seg of segments) {
        const segStat = await fs.stat(path.join(dir, seg));
        if (segStat.mtimeMs > newestMtime) newestMtime = segStat.mtimeMs;
      }
      ageMs = Date.now() - newestMtime;
      if (ageMs <= maxAgeMs) {
        return { alive: true, age_ms: ageMs, path: filePath, via: 'segment' };
      }
    }
...
export async function probeStream(url, timeout = 10000, userAgent = 'StreamRelay-HealthCheck/1.0') {
  ...
  try {
    ...
    clearTimeout(timer);
    if (response.ok) {
      return { alive: true, status: response.status, latency_ms: 0 };
    }
  } catch {
    clearTimeout(timer);
  }
  ...
  try {
    ...
    const isHls = body.includes('#EXTM3U') || body.includes('#EXT-X-') || body.length > 64;
    return {
      alive: response.ok && isHls,
      status: response.status,
      latency_ms: Date.now() - start,
      error: response.ok && !isHls ? 'Response is not a valid stream' : undefined,
    };
  } catch (err) {
