...
import { probeStream, checkProcessAlive } from '../utils/metrics.js';
...
export async function checkChannelHealth(channel) {
  ...
  if (channel.output_format === 'hls') {
    const hlsUrl = `http://127.0.0.1:${config.port}/hls/${channel.slug}/index.m3u8`;
    const probe = await probeStream(hlsUrl, 8000);
    result.checks.hls = probe;
    result.healthy = procAlive && probe.alive;
  } else {
