...
function buildFFmpegArgs(channel, sourceOverride = null, opts = {}) {
  ...
  // بثّ مباشر يُعاد بثّه (live restream).
  const isLiveRelay = ['hls', 'http', 'm3u'].includes(inputType);

  // تصحيح مهم بناءً على دراسة طريقة عمل Xtream-UI للقنوات المباشرة:
  //   • Xtream لا يستخدم -re أبداً للبث المباشر — يستخدمه فقط لملفات الأفلام (VOD).
  //   • للبث المباشر يستخدم -c copy وتمرير MPEGTS كما هو (passthrough).
  // كنا أضفنا -re للبث المباشر سابقاً ظناً أنها تقنية Xtream، وهذا خطأ:
  // توثيق FFmpeg نفسه يحذّر من -re عندما يكون المصدر مباشراً (realtime source).
  // وهي السبب الكلاسيكي لعرض سلس ثم تقطيع/تكرار المشهد/تجمّد بعد دقيقة أو دقيقتين
  // (انحراف ساعة -re عن ساعة المصدر المباشر → نقص في المخزن المؤقت). لذلك أزلناها.
  //
  // ملاحظة: لا نستخدم reconnect_at_eof أبداً — يسبب حلقة إعادة اتصال لحظية (0s) لا
  // نهائية عند أي مصدر يُرجع EOF بسرعة فلا يخرج فيديو وتعلّق القناة. نكتفي بإعادة
  // الاتصال عند أخطاء الشبكة الحقيقية فقط، وعندها EOF النظيف يُنهي العملية فوراً
  // فتفشل القناة بسرعة وتُجرَّب القناة الاحتياطية إن وُجدت.
  if (isLiveRelay) {
    // تجاهل أخطاء الإشارة بدل توقّف/تعليق العملية (تقنية Xtream، تُطبّق قبل ‎-i)
