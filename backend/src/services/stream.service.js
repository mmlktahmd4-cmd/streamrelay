...
function buildFFmpegArgs(channel, sourceOverride = null) {
  ...
  const inputType = channel.source_type || 'hls';

  if (inputType === 'hls' || inputType === 'm3u') {
    // HLS: لا نستخدم reconnect_at_eof لأن HLS demuxer يعالج EOF بشكل مختلف
    // reconnect_at_eof يسبب reconnect loop عند نهاية كل HTTP response وقد يوقف العملية
    args.push(
      '-reconnect', '1',
    ...
    // HTTP/RTMP streams: reconnect_at_eof مفيد هنا
    args.push(
      '-reconnect', '1',
      '-reconnect_streamed', '1',
      '-reconnect_at_eof', '1',
...
async function waitForHlsReady(channelId, procPid, timeoutMs) {
  ...
  const manifest = path.join(config.streaming.hlsDir, channelId, 'index.m3u8');
