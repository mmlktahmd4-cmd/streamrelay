...
function normalizeMetadata(input = {}) {
  ...
  if (meta.internal_hls_base_url) {
    meta.internal_hls_base_url = String(meta.internal_hls_base_url).trim().replace(/\/$/, '');
  }
  return meta;
}
...
export function isServerOnline(server, now = Date.now()) {
  ...
  const collectedAt = meta.host_stats?.collected_at;
  if (collectedAt) {
    const ts = new Date(collectedAt).getTime();
    if (Number.isFinite(ts) && (now - ts) <= HEARTBEAT_ONLINE_MS) return true;
  }
  ...
  return !server.last_heartbeat && server.hostname === config.serverId;
}
...
export async function listServers({ includeInactive = false } = {}) {
  ...
  return result.rows.map((row) => {
    ...
    const enriched = liveCount != null ? { ...row, current_streams: liveCount } : row;
...
export async function heartbeatLocalServer(activeCount = null) {
  ...
  // حاوية API-only لا ترسل heartbeat — يكفي worker البث
  if (config.serverRole === 'api-only') return local;
  ...
  if (serverIp && serverIp !== '127.0.0.1' && !serverIp.startsWith('172.')) {
    ...
    metadata.internal_hls_base_url = `http://${serverIp}:${httpPort}/api/hls`;
    metadata.stream_http_port = httpPort;

    // الدومين للوحة الرئيسية فقط — سيرفر البث البعيد يبقى على IP:port داخلياً
    const isRemoteStreamWorker = local.role === 'stream-only';
    if (urls.publicDomain && !isRemoteStreamWorker) {
      metadata.hls_base_url = urls.hlsBase;
      metadata.public_base_url = urls.baseUrl;
    } else {
      metadata.hls_base_url = metadata.internal_hls_base_url;
...
/** رابط HLS الداخلي — للاتصال المباشر بسيرفر البث (relay يستخدم IP:port البعيد) */
function isPublicDomainHost(hostname) {
  if (!hostname) return false;
  const urls = getPublicUrls();
  if (!urls.publicDomain) return false;
  const h = hostname.toLowerCase();
  const d = urls.publicDomain.toLowerCase();
  return h === d || h === `www.${d}`;
}

function isPublicDomainUrl(url) {
  try {
    return isPublicDomainHost(new URL(url).hostname);
  } catch {
    return false;
  }
}

function resolveRemoteStreamHttpPort(server) {
  const meta = normalizeMetadata(server?.metadata);
  if (meta.stream_http_port) {
    const p = parseInt(meta.stream_http_port, 10);
    if (p > 0) return p;
  }
  for (const url of [meta.internal_hls_base_url, meta.hls_base_url]) {
    if (!url || isPublicDomainUrl(url)) continue;
    try {
      const u = new URL(url);
  ...
  return parseInt(process.env.REMOTE_STREAM_HTTP_PORT || '8080', 10);
}

export function getInternalHlsBaseForServer(server) {
  if (server && (server.hostname === config.serverId || server.is_local)) {
    const ip = server?.ip_address ? String(server.ip_address).trim() : '';
    const port = parseInt(process.env.STREAMRELAY_HTTP_PORT || String(getPublicUrls().webPort || 80), 10);
    if (ip) return `http://${ip}:${port}/api/hls`;
    const urls = getPublicUrls();
    if (urls.publicDomain) {
      return `http://${urls.serverIp || '127.0.0.1'}:${port}/api/hls`;
    }
    return urls.hlsBase.replace(/^https:/i, 'http:');
  }

  const meta = normalizeMetadata(server?.metadata);
  if (meta.internal_hls_base_url && !isPublicDomainUrl(meta.internal_hls_base_url)) {
    return meta.internal_hls_base_url;
  }

  const ip = server?.ip_address ? String(server.ip_address).trim() : '';
  if (ip) {
    const port = resolveRemoteStreamHttpPort(server);
    return `http://${ip}:${port}/api/hls`;
  }

  const configured = meta.hls_base_url || null;
  if (configured && !isPublicDomainUrl(configured)) return configured;

  return getPublicUrls().hlsBase.replace(/^https:/i, 'http:');
}

/** إصلاح metadata السيرفرات البعيدة — يزيل رابط الدومين الخاطئ من hls_base_url */
export async function repairStreamServerInternalUrls() {
  const result = await query(
    `SELECT id, ip_address, metadata FROM servers
     WHERE is_active = true AND role = 'stream-only'`
  );
  let repaired = 0;
  for (const row of result.rows) {
    ...
    if (!ip) continue;
    const port = resolveRemoteStreamHttpPort(server);
    const internal = `http://${ip}:${port}/api/hls`;
    const meta = normalizeMetadata(row.metadata);
    const needsFix = isPublicDomainUrl(meta.hls_base_url || '')
      || meta.internal_hls_base_url !== internal
      || meta.stream_http_port !== port;
    if (!needsFix) continue;
    await patchServerMetadata(server.id, {
      internal_hls_base_url: internal,
      stream_http_port: port,
      hls_base_url: internal,
      public_base_url: `http://${ip}:${port}`,
    });
    repaired += 1;
  }
  if (repaired > 0) {
    log.info({ repaired }, 'Repaired remote stream server internal HLS URLs');
  }
  return repaired;
}

export function getPublicHlsBaseForServer(server) {
  const urls = getPublicUrls();
  if (urls.publicDomain) {
    return urls.hlsBase;
  }
  ...
  return getInternalHlsBaseForServer(server);
}

/** @deprecated استخدم getPublicHlsBaseForServer — للتوافق */
export function getHlsBaseForServer(server) {
  return getPublicHlsBaseForServer(server);
}
...
export async function resolveHlsBaseForChannel(channel) {
  ...
  if (channel.server_id) {
    ...
    if (server) return getPublicHlsBaseForServer(server);
  }
...
export async function assignServerForChannel(channelId) {
  ...
  if (channel.server_id) {
    ...
    if (!pinned.online) {
      log.warn({ hostname: pinned.hostname, channelId }, 'Pinned server stale heartbeat — queueing anyway');
    }
...
export async function bindChannelToServer(channelId, server, slug) {
  const local = await getLocalServer();
  if (local && server.id !== local.id) {
    const { purgeLocalHlsCache } = await import('./hls-relay.service.js');
    await purgeLocalHlsCache(channelId, slug);
  }

  const hlsBase = getPublicHlsBaseForServer(server);
  await query(
