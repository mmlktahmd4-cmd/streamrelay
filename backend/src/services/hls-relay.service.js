...
export async function handleHlsRelay(request, reply) {
  ...
  if (!upstream) {
    return reply.status(404).send({ error: 'Channel HLS not available' });
  }
  ...
  try {
    ...
    log.warn({ slug, file, upstream, err: err.message }, 'HLS relay fetch failed');
    return reply.status(502).send({ error: 'Cannot reach stream server HLS' });
  }
}
