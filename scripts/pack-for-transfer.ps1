# StreamRelay - pack for Ubuntu 22/24 transfer
$ErrorActionPreference = "Stop"
$Root = Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path)
Set-Location $Root

$Out = if ($args[0]) { $args[0] } else { "streamrelay-ubuntu.tar.gz" }

Write-Host "=== Packing StreamRelay for Ubuntu 22/24 ===" -ForegroundColor Cyan
