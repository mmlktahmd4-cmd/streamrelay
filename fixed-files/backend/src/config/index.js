...
export const config = {
  ...
  streaming: {
    ...
    healthCheckInterval: parseInt(process.env.HEALTH_CHECK_INTERVAL || '45', 10) * 1000,
    maxRestartAttempts: parseInt(process.env.MAX_RESTART_ATTEMPTS || '10', 10),
    restartCooldown: parseInt(process.env.RESTART_COOLDOWN || '8', 10) * 1000,
    startupMinWaitMs: parseInt(process.env.STARTUP_MIN_WAIT_SEC || '3', 10) * 1000,
    hlsReadyTimeoutMs: parseInt(process.env.HLS_READY_TIMEOUT_SEC || '120', 10) * 1000,
    startupRecoveryDelayMs: parseInt(process.env.STARTUP_RECOVERY_DELAY_SEC || '5', 10) * 1000,
    recoveryStaggerMs: parseInt(process.env.STREAM_RECOVERY_STAGGER_MS || '2000', 10),
    onDemandIdleTimeoutSec: parseInt(process.env.ON_DEMAND_IDLE_TIMEOUT_SEC || '600', 10),
    onDemandViewerPulseSec: parseInt(process.env.ON_DEMAND_VIEWER_PULSE_SEC || '120', 10),
    onDemandIdleCheckSec: parseInt(process.env.ON_DEMAND_IDLE_CHECK_SEC || '60', 10),
    streamWatchdogIntervalMs: parseInt(process.env.STREAM_WATCHDOG_INTERVAL_SEC || '60', 10) * 1000,
    bulkStartStaggerMs: parseInt(process.env.BULK_START_STAGGER_MS || '2000', 10),
  },
