...
export default function ServersPage() {
  ...
  const messageBannerClass = messageTone === 'error'
  ...
  const handleSelfUpdate = async () => {
    if (!confirm('تنزيل آخر تحديث من GitHub وتثبيته على هذا السيرفر؟\nقد تنقطع اللوحة لدقيقة أو أكثر ثم تعود تلقائياً.')) return;
    setUpdating(true);
    setMessage('');
    try {
      const { data } = await triggerSelfUpdate();
      setMessage(data.message || 'بدأ التحديث...');
    } catch (err) {
      setUpdating(false);
  ...
  useEffect(() => { loadServers(); loadUpdateInfo(true); }, []);

  useEffect(() => {
    loadServers();
  }, [showInactive]);

  // متابعة حالة التحديث (أسرع أثناء التشغيل لرصد النجاح/الفشل وعودة اللوحة)
  useEffect(() => {
    const interval = setInterval(() => loadUpdateInfo(true), updating ? 5000 : 60000);
  ...
  const formatRemoteSyncMessage = (data) => {
    if (data.skipped) {
      return { text: 'التحديث التلقائي للبعيد معطّل (AUTO_UPDATE_REMOTES=0)', tone: 'warning' };
    }
    if ((data.total || 0) === 0 && data.message) {
      return { text: data.message, tone: 'warning' };
    }
    if ((data.updated || 0) === 0 && (data.failed || 0) === 0) {
      return {
        text: 'لا توجد سيرفرات بعيدة ببيانات SSH — أضف SSH من بطاقة السيرفر',
        tone: 'warning',
      };
    }
    ...
    const lines = [];
    if (data.failed > 0) {
      lines.push(`فشل ${data.failed} من ${data.total || 0} سيرفر`);
    }
    if (data.updated > 0) {
      lines.push(
        data.updated === 1
          ? '✓ تم تحديث السيرفر البعيد من GitHub بنجاح'
          : `✓ تم تحديث ${data.updated} سيرفرات بعيدة من GitHub بنجاح`
      );
    }
    ...
    const details = (data.results || [])
      ...
      .map((r) => {
        const name = r.hostname || r.server_id?.slice(0, 8) || '—';
        return r.commit ? `${name} → ${String(r.commit).slice(0, 12)}` : name;
      });
    if (details.length) lines.push(details.join(' · '));

    const errors = (data.results || [])
      .filter((r) => r.status === 'failed' && r.error)
      .map((r) => `${r.hostname || 'سيرفر'}: ${r.error}`);
    if (errors.length) lines.push(...errors);

    lines.push('ملاحظة: هذا يحدّث سيرفر البث البعيد فقط — لتحديث اللوحة الرئيسية استخدم «تحديث اللوحة من GitHub»');

    let tone = 'success';
    if (data.failed > 0 && data.updated > 0) tone = 'warning';
    else if (data.failed > 0) tone = 'error';

    return { text: lines.join('\n'), tone };
  };

  const handleSyncAllRemotes = async () => {
    if (!confirm('تحديث كل السيرفرات البعيدة التي لها بيانات SSH؟\n\n• git pull على السيرفر البعيد\n• إعادة بناء worker\n• لا يحدّث اللوحة الرئيسية\n\nقد يستغرق عدة دقائق.')) return;
    setSaving(true);
    showMessage('', 'success');
    try {
      const { data } = await syncRemoteServers();
      const result = formatRemoteSyncMessage(data);
      showMessage(result.text, result.tone);
      loadServers();
      loadUpdateInfo(true);
    } catch (err) {
      showMessage(err.response?.data?.error || 'فشل تحديث السيرفرات البعيدة', 'error');
    }
    setSaving(false);
  };
    ...
    <div>
      <div className="mb-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {hasPendingUpdate && (
            <span className="inline-flex items-center gap-1.5 rounded-lg bg-amber-500 px-3 py-2 text-sm font-bold text-white shadow-md animate-pulse">
              <BellRing className="w-4 h-4" />
              {remoteUpdate.behind_by} تحديث جديد على GitHub
            </span>
          )}
          {updateInfo?.os_apply_available ? (
            <button
              type="button"
              className={`btn relative ${hasPendingUpdate ? 'btn-primary ring-2 ring-amber-400 ring-offset-2' : 'btn-primary'}`}
              disabled={updating}
              onClick={handleSelfUpdate}
              title={hasPendingUpdate ? `تحديث جديد: ${remoteUpdate?.latest_subject || ''}` : 'git pull + إعادة بناء اللوحة على هذا السيرفر'}
            >
              {hasPendingUpdate && (
                <span className="absolute -top-2 -right-2 flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-amber-500 px-1 text-[10px] font-bold text-white shadow">
                  {remoteUpdate.behind_by > 9 ? '9+' : remoteUpdate.behind_by}
                </span>
              )}
              {hasPendingUpdate ? <BellRing className="w-4 h-4 animate-pulse" /> : <DownloadCloud className={`w-4 h-4 ${updating ? 'animate-bounce' : ''}`} />}
              {updating
                ? 'جاري تحديث اللوحة...'
                : hasPendingUpdate
                  ? `تحديث جديد (${remoteUpdate.behind_by})`
                  : 'تحديث اللوحة من GitHub'}
            </button>
          ) : hasPendingUpdate ? (
            <button type="button" className="btn btn-primary ring-2 ring-amber-400 ring-offset-2" disabled title="حدّث يدوياً على السيرفر">
      ...
      {message && !mode && !sshTarget && (
        <p className={`mb-4 text-sm px-4 py-2 rounded-lg whitespace-pre-line font-medium ${messageBannerClass}`}>
          {message}
        </p>
      )}

      {updateInfo && (
        <div className={`mb-4 card overflow-hidden ${hasPendingUpdate ? 'border-amber-300 bg-amber-50/40' : ''}`}>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 py-3 px-4">
            <div className="text-sm flex-1">
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-bold text-slate-800">تحديث اللوحة</span>
                {hasPendingUpdate && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-amber-500 px-2 py-0.5 text-[11px] font-bold text-white">
                    <BellRing className="w-3 h-3" /> تحديث جديد متاح
                  </span>
                )}
              </div>
              {updateInfo.current_commit && (
                <p className="text-slate-500 mt-1">
                  النسخة الحالية: <span className="font-mono">{updateInfo.current_commit}</span>
                  {remoteUpdate?.remote_commit && hasPendingUpdate && (
                    <> → الأحدث: <span className="font-mono text-amber-800">{remoteUpdate.remote_commit}</span></>
                  )}
                  {!hasPendingUpdate && remoteUpdate?.remote_commit && !remoteUpdate?.check_skipped && !remoteUpdate?.check_error && (
                    <> — GitHub: <span className="font-mono text-emerald-700">{remoteUpdate.remote_commit}</span> (محدّث)</>
                  )}
                </p>
              )}
              {!hasPendingUpdate && !remoteUpdate?.check_skipped && !remoteUpdate?.check_error && remoteUpdate?.checked_at && (
                <p className="text-xs text-emerald-600 mt-1">اللوحة محدّثة مع GitHub</p>
              )}
              {remoteUpdate?.check_skipped && (
                <p className="text-xs text-amber-700 mt-1 font-medium">تعذّر التحقق: {remoteUpdate.reason}</p>
              )}
              ...
              {!updateInfo.os_apply_available && (
                <p className="text-xs text-amber-700 mt-1">
                  التحديث بضغطة يتطلب الوصول للجهاز — حدّث مرة واحدة يدوياً: <span className="font-mono">cd /opt/streamrelay && sudo bash scripts/deploy-update.sh</span>
                </p>
              )}
              {remoteUpdate?.check_error && (
                <p className="text-xs text-red-600 mt-1 font-medium">تعذّر التحقق من GitHub: {remoteUpdate.check_error}</p>
              )}
            </div>
            <div className="flex items-center gap-2 shrink-0">
              ...
              {hasPendingUpdate && (remoteUpdate?.commits?.length > 0) && (
                ...
                >
                  {showUpdateDetails ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                  {showUpdateDetails ? 'إخفاء التفاصيل' : 'عرض الميزات'}
                </button>
              )}
              {updating && <RefreshCw className="w-5 h-5 text-cyan-600 animate-spin" />}
            </div>
          </div>

          {hasPendingUpdate && showUpdateDetails && remoteUpdate?.commits?.length > 0 && (
            <div className="border-t border-amber-200/80 bg-white/70 px-4 py-3 max-h-64 overflow-y-auto">
              <p className="text-xs font-bold text-slate-600 mb-2">
                التحديثات الجديدة ({remoteUpdate.behind_by}) — لم تُرفَع بعد على هذا السيرفر
              </p>
              <ul className="space-y-2">
                {remoteUpdate.commits.map((c) => (
                  <li key={c.hash} className="text-sm border-r-2 border-amber-400 pr-3">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="font-mono text-xs text-slate-500">{c.hash}</span>
                      <span className="font-medium text-slate-800">{c.subject}</span>
                    </div>
                    {c.body && (
                      <p className="text-xs text-slate-500 mt-0.5 whitespace-pre-wrap">{c.body}</p>
                    )}
                  </li>
                ))}
              </ul>
