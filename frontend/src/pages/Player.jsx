...
export default function Player() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setChannel(ch);
        if (ch.content_type !== 'vod' && (ch.on_demand || ch.status === 'running')) {
          try {
            const { data: url } = await getPlaybackUrl(id);
            setPlaybackUrl(url.url);
            setStreamStarting(!!url.starting);
          } catch { /* ignore */ }
        }
      } catch { /* ignore */ }
      setLoading(false);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      {(channel.status === 'running' || channel.on_demand) && playbackUrl ? (
        <div className="space-y-4">
          <div className="rounded-lg overflow-hidden border border-slate-200 shadow-sm">
            <HlsPlayer
              src={playbackUrl}
              onDemand={!!channel.on_demand}
        ...
        <div className="aspect-video rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-400">
          {channel.status === 'running' || channel.on_demand ? 'جاري تحميل البث...' : 'القناة غير نشطة — شغّل القناة أولاً'}
        </div>
      )}
