  ...
  useEffect(() => {
    ...
    function initHls() {
      ...
      const hls = new Hls({
        ...
        // بالثواني لا بعدد المقاطع: مقاطع ABR صارت 2ث، فالعدّ بالمقاطع (4) كان سيعطي
        // 8ث فقط من المخزن — غير كافٍ لامتصاص انقطاعات الواي-فاي الضعيف. 20ث ≈ نفس
        // القيمة السابقة لقنوات copy (4 × 6ث) ومخزن مريح لقنوات ABR.
        liveSyncDuration: 20,
        liveMaxLatencyDuration: 120,
        // تقدير سرعة النت الابتدائي منخفض عمداً (سلوك يوتيوب): أول مقطع يُطلب من أدنى
        // درجة (144p ≈ 200 kbps) فتظهر الصورة خلال ثانية حتى على إشارة ضعيفة، ثم
        // يرفع المشغّل الجودة تلقائياً بعد قياس السرعة الحقيقية من المقاطع الأولى.
        abrEwmaDefaultEstimate: 300000,
        // رفع الجودة بتحفّظ (0.6 من السرعة المقاسة) وخفضها بسرعة (0.8): يمنع القفز
        // لجودة أعلى ثم التجمّد — أفضل جودة أقل ثابتة من جودة أعلى متقطعة.
